...
# ==============================================================================
st.set_page_config(
    page_title="Quant Factor Lab Pro v3.5 (Brapi Fix)",
    layout="wide",
...
def fetch_fundamentals_brapi(tickers: list, token: str) -> pd.DataFrame:
    ...
    Correção v3.5: Melhor tratamento de JSON e Autenticação.
    """
    # Remove sufixos .SA se existirem para a chamada Brapi, e remove benchmarks
    ...
    # Estratégia de busca profunda em dicionários aninhados
    def get_nested_val(item, keys_list):
        """Tenta encontrar um valor verificando vários caminhos possíveis."""
        # 1. Tenta direto na raiz (comum na Brapi simplificada)
        for key in keys_list:
            if key in item and item[key] is not None:
                return item[key]
        
        # 2. Tenta nos módulos comuns do Yahoo/Brapi
        sub_modules = ['defaultKeyStatistics', 'financialData', 'summaryProfile', 'price']
        for module in sub_modules:
            if module in item and isinstance(item[module], dict):
                for key in keys_list:
                    if key in item[module] and item[module][key] is not None:
                        # Às vezes vem como objeto {'raw': ... , 'fmt': ...}
                        val = item[module][key]
    ...
    for i, ticker in enumerate(clean_tickers):
        ...
        # Parâmetros ajustados para garantir autenticação e dados fundamentais
        params = {
            'token': token,       # Token na URL é mais robusto para alguns endpoints
            ...
            if response.status_code == 200:
                ...
                for item in results:
                    try:
                        ...
                        # Extração de Preço e Valor de Mercado
                        price = safe_float(item.get('regularMarketPrice'))
                        market_cap = safe_float(item.get('marketCap'))
                        
                        # Setor (Tenta raiz, depois summaryProfile)
                        sector = item.get('sector')
                        if not sector:
                            sector = item.get('summaryProfile', {}).get('sector', 'Outros')

                        # Extração Resiliente de Indicadores
                        # Tenta nomes variados que a API pode retornar
                        ...
                        st.warning(f"Erro ao processar dados de {ticker}: {e_parse}")
            
            elif response.status_code == 429:
                st.warning(f"Limite atingido em {ticker}. Aguardando...")
                time.sleep(5) 
            elif response.status_code == 404:
                # Ticker não encontrado na Brapi, apenas ignora
                pass
            else:
                st.error(f"Erro Brapi {ticker}: Status {response.status_code}")
                
        except Exception as e_req:
            st.error(f"Erro de conexão com {ticker}: {e_req}")
        
        # Atualiza barra
        progress_bar.progress((i + 1) / total_tickers)
        
        # Delay CRUCIAL para plano gratuito
        time.sleep(1.2) 
...
def main():
    st.title("🧪 Quant Factor Lab: Pro v3.5 (Brapi Fix)")
    st.markdown("""
    **Otimização Multifator Institucional**
    * **Ranking Atual:** Dados da API Brapi.dev (Value, Quality, Momentum).
    * **Correção:** JSON parsing ajustado para o novo formato da API e token passado via query params.
    """)
